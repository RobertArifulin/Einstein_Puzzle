...
def solve_puzzle(conditions, questions, table, deep):
    ...
    if uncompleted_cells > 0 and not conditions:
        ...
        if is_solved:
            return True, answer.copy(), copy.deepcopy(table)
        else:
            return False, None, None
...
def removing_excess(conditions, questions, table, deep):
    ...
    is_solved, new_answers, new_table = solve_puzzle(new_conditions.copy(), questions.copy(), copy.deepcopy(empty_table), deep)
    if not is_solved:
        print(is_solved)
        pass
...
def brute_force_search(table, conditions, questions, deep):
    """
    ...
    if not deep and (len(conditions) != 1 or conditions[0][0] != '3'):  # Выход из рекурсии
    ...
    for y in range(height):
        for x in range(width):
            if 1 < len(new_table[y][x]) <= min_k:  # Находим клетку с мин кол-вом вариантов.
                min_k = len(new_table[y][x])
                new_cell = (x, y)

    for option in table[new_cell[1]][new_cell[0]]:  # Перебор вариантов.
        backup = copy.deepcopy(new_table)  # Запоминаем состояние таблицы.
        ...
        is_solved, answer, new_table = solve_puzzle(conditions.copy(), questions.copy(), new_table, deep - 1)  # Пытаемся решить
    ...
    if number_of_solution == 1:
        return True, copy.deepcopy(final_table), deep, final_answer.copy()
    return False, None, deep, None
...
def generate_puzzle(width, height, deep):
    ...
    empty_table = copy.deepcopy(table)
    # is_solved, new_answers1, new_table = solve_puzzle(conditions.copy(), questions.copy(), table, deep)
    ...
    print(len(conditions) - 1, is_solved)
